...
BC_fun_Top, BC_A_Top = BC.getUBc([1,0], 0.0, 0.0)
...
for it in range(nt):
    ...
    ### inlet
    #f_in0[:,0,:] = applyWideBC(BC_A_Bottom, f_in0[:,0,:])
    ### outlet
    #f_in0[:,-1,:] = applyWideBC(BC_A_Top, f_in0[:,-1,:])
    
    ### bottom
    f_in0[0,:,:] = applyWideBC(BC_A_Left, f_in0[0,:,:])
    ### top
...
#==============================================================================
    ...
    if it > 0 and np.mod(it, nt/10) == 0:
        iii = 0
        T= np.array(test(x0,y0))
        print T.shape
        plt.contourf(x0,y0,(U0[:,:,0]-T[0,:,:])**2 + (U0[:,:,1]-T[1,:,:])**2) 
        plt.colorbar()
